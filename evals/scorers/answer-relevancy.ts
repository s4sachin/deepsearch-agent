...
// Prompt for breaking down the answer into statements
...
export const checkAnswerRelevancy = async (opts: {
  ...
  const { object: statementsResult } = await generateObject({
    model: relevancyModel,
    prompt: generateEvaluationStatementsPrompt({ output: opts.answer }),
    schema: z.object({
      statements: z.array(z.string()).describe("Array of statements"),
    }),
  });
  ...
  if (statements.length === 0) {
    return {
      ...
      metadata: {
        statements: [],
        verdicts: [],
        summary: "No statements to evaluate",
      },
    };
  }
  ...
  const { object: verdictsResult } = await generateObject({
    ...
    schema: z.object({
      verdicts: z.array(
        z.object({
          verdict: z.enum(["yes", "no", "unsure"]).describe("The verdict"),
  ...
  const totalScore = verdicts.reduce(
    (sum, verdict) => sum + scoreMap[verdict.verdict],
    0
  );
  ...
  return {
    ...
    metadata: {
      statements,
      verdicts,
      summary: `${verdicts.filter((v) => v.verdict === "yes").length} yes, ${verdicts.filter((v) => v.verdict === "unsure").length} unsure, ${verdicts.filter((v) => v.verdict === "no").length} no out of ${verdicts.length} statements`,
    },
  };
};
